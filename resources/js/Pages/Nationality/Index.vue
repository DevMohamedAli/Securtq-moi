...
const new_nationality_form = reactive({
    'name': '   ',

});

Boolean.submited = (value) => {
    if (value) {
        Swal.fire({
            title: 'تم إضافة جنسية جديدة',
            text: 'تم إضافة جنسية جديدة بنجاح',
            icon: 'success',
            confirmButtonText: 'حسناً'
        })
    }
    const model = getElementById("new-nationality-model");
    model.setAttribute("hidden", true);
}

const submit = (url, form) => {
...
<template>
    ...
    <AuthLayout>
        <div
            ...
            <h2 class="font-medium text-gray-900 dark:text-white">الجنسيات</h2>
            <div class="flex flex-row-reverse justify-between items-center">
                <button data-modal-target="new-nationality-model" data-modal-toggle="new-nationality-model"
                    ...
                    <span class="mr-2">إضافة جنسية</span>
                </button>
            </div>

        </div>
        <!-- Main modal -->
        <div id="new-nationality-model" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">إضافة جنسية</h3>
                        <p class="text-sm text-gray-700 dark:text-gray-400">
                            يرجى ملئ الحقول التالية
                        </p>
                        <form @submit.prevent="submit('nationalities.store', new_nationality_form)
                            " class="mt-4">
                            <div class="relative z-0 w-full mb-6 group">
                                <input type="name" id="name" v-model="new_nationality_form.name"
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr v-for="nationality in nationalities" :key="nationality.id" class=" bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50
                        ...
                        <td class="px-6 py-4 text-right">
                            <Link :href="route('nationalities.edit', nationality.id)" class="icon-style relative">
                            تعديل
